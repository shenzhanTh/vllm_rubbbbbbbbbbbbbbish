...
class PagedAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
                key_cache: Optional[torch.Tensor], value_cache: Optional[torch.Tensor],
        ...
        if input_metadata.is_prompt:
            if (key_cache is None or value_cache is None or input_metadata.block_tables.numel() == 0):
                ...
                if self.use_ref_attention:
                    output = self.ref_masked_attention(query, key, value)
                    return output.reshape(batch_size, seq_len, hidden_size)
                ...
                context_attention_fwd(query, key, value, output, key_cache, value_cache,
                                       input_metadata.block_tables, input_metadata.start_loc,
                                       input_metadata.prompt_lens, input_metadata.context_lens,
                                       input_metadata.max_seq_len, getattr(self, "alibi_slopes", None))
        else:
...
def _make_alibi_bias(alibi_slopes: torch.Tensor, num_kv_heads: int, batch_size: int,
                     seq_len: int, dtype: torch.dtype) -> LowerTriangularMaskWithTensorBias:
    bias = torch.arange(seq_len, dtype=dtype)
    bias = bias[None, :] - bias[:, None]
...
def _paged_attention(query: torch.Tensor, key_cache: torch.Tensor,
    ...
    if use_v1:
        ops.paged_attention_v1(output, query, key_cache, value_cache, num_kv_heads, scale,
                               input_metadata.block_tables, input_metadata.context_lens,
                               block_size, input_metadata.max_context_len, alibi_slopes,
                               input_metadata.kv_cache_dtype)
    else:
        assert _PARTITION_SIZE % block_size == 0
        tmp_output = torch.empty(size=(num_seqs, num_heads, max_num_partitions, head_size),
