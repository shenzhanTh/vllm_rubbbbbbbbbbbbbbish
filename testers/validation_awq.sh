...
cd /opt/opencompass
output=$(python /opt/opencompass/run.py configs/vllm/eval_llama3_awq_vllm.py)
...
generate_throughput_output=$(python benchmark_throughput.py -q awq --num-prompts 32 --model  /models/llama3/Meta-Llama-3-8B-AWQ --dataset /models/llama3/data/ShareGPT_V3_unfiltered_cleaned_split.json --trust-remote-code  --enforce-eager --dtype float16 | tail -n 1 )
